...
?>
        ...
        <div ng-show="region.widgetTags.lastDonorAndDonation" ng-init="tagPreference = region.widgetTags.lastDonorAndDonationWidget" child-scope>
            <?= $this->render('partials/region-tag', array('content' =>'
                Last Donor/Donation&nbsp;:&nbsp;<span id="last_donor_donation">{{donationsService.stats.last_donor.name}} (${{number_format(donationsService.stats.last_donor.amount,2)}})</span>
            ')) ?>
        </div>
        ...
        <div ng-show="region.widgetTags.largestDonation" ng-init="tagPreference = region.widgetTags.largestDonationWidget" child-scope>
            <?= $this->render('partials/region-tag', array('content' =>'
                Largest Donation&nbsp;:&nbsp;<span id="largest_donation">${{number_format(donationsService.stats.top_donation.amount,2)}} ({{donationsService.stats.top_donation.displayName}})</span>
            ')) ?>
        </div>
        ...
        <div ng-show="region.widgetTags.topDonor" ng-init="tagPreference = region.widgetTags.topDonorWidget" child-scope>
            <?= $this->render('partials/region-tag', array('content' =>'
                Top Donor&nbsp;:&nbsp;<span id="top_donor">{{donationsService.stats.top_donors[0].name}} (${{number_format(donationsService.stats.top_donors[0].amount,2)}})</span>
            ')) ?>
        </div>
