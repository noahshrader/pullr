<? if (count($donors) > 0 || ($showSubscriber && count($subscribers) > 0 ) || ($showFollower && count($followers) > 0)): ?>
	<? foreach($donors as $index => $donor): ?>                    
	     <?=$donor['name']; ?> ($<?=\common\components\NumberUtils::formatNumber($donor['amount'], 2);?>)<? if ($index < count($donors) - 1 || ($showSubscriber && count($subscribers) > 0) || ($showFollower && count($followers) > 0)):?>,<? endif; ?>
	<? endforeach; ?>
