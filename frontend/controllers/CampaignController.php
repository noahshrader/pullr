...
class CampaignController extends FrontendController {
    ...
    public function actionDonors($viewDonorParams = null){
        ...
        $sql = 'SELECT email, SUM(amount) sum, GROUP_CONCAT(DISTINCT nameFromForm SEPARATOR " ") nameFromForm, firstName, lastName FROM '.Donation::tableName().' WHERE userId = '.$user->id.' AND paymentDate > 0 AND email<>"" GROUP BY email, lastName, firstName ORDER BY sum DESC';
